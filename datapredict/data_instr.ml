...
let instr_cov_ht = Hashtbl.create 4096
...
(* every site has a location, overall statement number, site number, and predicate *)
type every_site = Cil.location * int * int * Cil.exp 

type site_info = 
	(* Branches : standard info, stmt of then branch, stmt num
	 * of else branch *)
	Branches of every_site * int * int
	 (* returns : standard info *)
  | Returns of every_site
	 (* scalar pairs: standard info, optional list of statements to associate
		with its predicate's value *)
  | Scalar_pairs of every_site * int list
...
let get_next_site scheme exp l sid = 
  let count = !site in
    incr site ;
    hadd site_ht count (l,scheme,sid,exp);
...
let instr_branch e1 l s =
  let _,str_exp = get_next_site "branches" e1 l s.sid in
...
let instr_rets e l s =
  ...
  let exp_and_conds = 
    List.map (fun cond -> 
		let _, s = get_next_site "returns" cond l s.sid in 
		  (s,cond)) conds in
  let instrs1 =
...
class instrumentVisitor = object(self)
  ...
  method print_vars lhs rhs sid = (* lval and rvals are exps *)
    let count,str = get_next_site "scalar-pairs" lhs !currentLoc sid in
  ...
  method vstmt s = 
    ...
    in
    let instrument_stmt s tv fn = if tv then insert_before (fn s) s else s 
    in
      ChangeDoChildrenPost
		(s, 
		 fun s -> 
  ...
  method vfunc fdec = hclear local_vars; DoChildren (* don't I want to
													   add function
													   parameters to
													   the local vars
													   table? *)
		...
		(fun filename -> 
		   let file = Frontc.parse filename () in
			 ...
			 let new_global = GVarDecl(stderr_va,!currentLoc) in 
			   ...
			   let new_stmt = {new_stmt with labels = if !dolabels then make_label() else new_stmt.labels} in 
