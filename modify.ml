...
let version = "EMS: Sun Aug 16 17:29:14 MDT 2009"

(* This file is used to signal to all processes that a solution has
   been found and they should exit, please change the path to something
   appropriate for your system.  Note that any file located at this path
   will be removed. *)
let scram_file = "/tmp/genprog-scram"
 ...
 * Utility Functions 
 ***********************************************************************)
let hashtbl_join (a : (Digest.t, float) Hashtbl.t) (b : (Digest.t, float) Hashtbl.t) =
  Hashtbl.iter (fun k v -> Hashtbl.replace b k v) a

let hashtbl_add_float (a : (string, float) Hashtbl.t) (b : (string, float) Hashtbl.t) =
  Hashtbl.iter (fun k v -> Hashtbl.replace b k (v +. (try Hashtbl.find b k with Not_found -> 0.0))) a

let hashtbl_add_int (a : (string, int) Hashtbl.t) (b : (string, int) Hashtbl.t) =
...
let divider lis chunk_size =
  let rec aux (part, whole) lis chunk_size =
    match lis with
      | [] -> part :: whole
      | head :: tail ->
          if (List.length part < chunk_size) then
            aux (head :: part, whole) tail chunk_size
          else
            aux ([head], part :: whole) tail chunk_size
  in aux ([], []) lis chunk_size

let combiner lis_of_liss =
  List.fold_left (fun tail head -> List.append head tail) [] lis_of_liss


(* 
 * maps a function along a list in s.t. each copy of the function runs
 * in it's own thread in parallel
 *
 * This is taken largely from an email posted at
 * http://groups.google.com/group/fa.caml/browse_thread/thread/e22b4eb77ce2ee73
 *)
let fork_map (f : 'a -> 'b) a : 'b list = 
...
let tracking_join a b =
  { current = counters_join a.current b.current ;
    at_last_fitness = counters_join a.at_last_fitness b.at_last_fitness
    ; }
...
(*
 * Four AST visitors used in crossover and mutation. 
 *)
...
let fitness (i : individual) (pll_fit : bool)
            (* returns *) : float = 
...
(* for easy debugging. *) 
    ...
    if fitness >= (float_of_int !max_fitness) then begin
      ...
      if better then begin 
        ...
        flush stdout ; 
        most_fit := Some(our_size, fitness, file, now, !fitness_count, copy tracking.current) ;
        if not !continue then begin
          (* touch our global indicator file *)
          let fout = open_out scram_file in 
            Marshal.to_channel fout !most_fit [] ;
            if (not pll_fit) then begin
              (* print out the information on the best *) 
              !print_best_output () ;
...
type fit_records =
    {
      with_fitness : (individual * float) list ;
      compile_fail : int ;
      total_avg : counters ;
      nonzerofitness_avg : counters ;
      total_fitness_evals : int ;
      total_nonzerofitness_evals : int ;
      fitness_ht : (Digest.t , float) Hashtbl.t ;
      stats2_invocations : (string, int) Hashtbl.t ;
      stats2_totals : (string , float) Hashtbl.t ;
      max_fitness : int ;
      most_fit : (int * float * Cil.file * float * int * counters) option ;
    }

(* a parallel variant of the current pop_with_fitness function *)
let parallel_pop_with_fitness
    (num_pll_fit : int)
    (population : individual list)
    (* returns *) : (individual * float) list =
  let do_group (group : individual list) : fit_records =
    let compile_fail = ref 0 in
    let total_avg = ref (new_counters()) in
    let nonzerofitness_avg = ref (new_counters ()) in
    let total_fitness_evals = ref 0 in
    let total_nonzerofitness_evals = ref 0 in
    let with_fitness_list = ref [] in
    let with_fitness = List.map (fun member ->
                                   if not(Sys.file_exists scram_file) then
                                     let f = fitness member true in
                                       (member, f)
                                   else
                                     (member, 0.0)
                                ) group in
      with_fitness_list := with_fitness ;
      (* return the results *)
      {
        with_fitness = !with_fitness_list ;
        compile_fail = !compile_fail ;
        total_avg = !total_avg ;
        nonzerofitness_avg = !nonzerofitness_avg ;
        total_fitness_evals = !total_fitness_evals ;
        total_nonzerofitness_evals = !total_nonzerofitness_evals ;
        fitness_ht = fitness_ht ;
        stats2_invocations = Stats2.invocations ;
        stats2_totals = Stats2.totals ;
        max_fitness = !max_fitness ;
        most_fit = !most_fit ;
      } in
    (* now actuall split into sub-pops and run forked fitness *)
    (combiner
       (List.map (fun my_fit_records ->
                    (* update global counters which were updated in child processes *)
                    compile_fail := !compile_fail + my_fit_records.compile_fail ;
                    total_avg := counters_join !total_avg my_fit_records.total_avg ;
                    nonzerofitness_avg := counters_join !nonzerofitness_avg my_fit_records.nonzerofitness_avg ;
                    total_fitness_evals := !total_fitness_evals + my_fit_records.total_fitness_evals ;
                    (* combine fitness hashes *)
                    hashtbl_join my_fit_records.fitness_ht fitness_ht ;
                    (* combine the Stats2 hashes *)
                    hashtbl_add_int my_fit_records.stats2_invocations Stats2.invocations ;
                    hashtbl_add_float my_fit_records.stats2_totals Stats2.totals ;
                    (* potentially update the max fitness and the most fit *)
                    (match my_fit_records.most_fit with
                       | None -> () ;
                       | Some(best_size, best_fitness, best_file, tau, best_count, tracking) ->
                           if (my_fit_records.max_fitness >= !max_fitness) then begin
                             max_fitness := my_fit_records.max_fitness ;
                             most_fit := my_fit_records.most_fit ;
                           end) ;
                    (* lets quit gracefully if a solution has already been found *)
                    if Sys.file_exists scram_file then begin
                      let scram_fin = open_in_bin scram_file in 
                        most_fit := Marshal.from_channel scram_fin ;
                        (* print out the information on the best *) 
                        !print_best_output () ;
                        Stats2.print stdout "Genetic Programming Prototype" ; 
                        Stats2.print !debug_out "Genetic Programming Prototype" ; 
                    ...
                    (* return the indidividuals with their fitness *)
                    my_fit_records.with_fitness)
          (fork_map (fun group ->
                       Stats2.clear () ;
                       let pop (x, y, z) = x in
                       let cc (x, y, z) = y in
                       let pc (x, y, z) = z in
                         compile_counter := cc group ;
                         port := pc group ;
                         do_group (pop group))
             (List.map (fun group ->
                          (* update global counters which will be updated in child processes *)
                          let size = List.length group in
                            compile_counter := !compile_counter + size ;
                            port := !port + size ;
                            (group, (!compile_counter - size), (!port - size)))
                (divider population (List.length population / num_pll_fit))))))
...
let ga_step (original : individual) 
   ...
   *) 
  let pop_with_fitness = 
  if num_pll_fit > 1 then begin
    parallel_pop_with_fitness num_pll_fit incoming_population
  end
    else
      List.map (fun member ->
                  let f = fitness member false in
                    (member,f)
             ) incoming_population
in 
...
let ga (indiv : individual) 
       ...
       (num_pll_fit : int)   (* number of sub-populations for parallel fitness *)
       (* returns *) : unit = 
  ...
  for i = 1 to generations do
    ...
    (* optionally evolve sub-populations in parallel *)
    population := ga_step indiv !population num num_pll_fit
  done 
 ...
 ***********************************************************************)
let main () = begin
  if Sys.file_exists scram_file then Unix.unlink scram_file ;
  let good_path_factor = ref 0.0 in 
  let generations = ref 10 in 
  let pll_fit = ref 1 in 
  let pop = ref 40 in 
  ...
  let argDescr = [
    ...
    "--pll_fit", Arg.Set_int pll_fit, "X distribute fitness evaluation over X processes (def: 1)";
  ...
  if !filename <> "" then begin
    ...
    ga (file,ht,count,path,new_tracking ()) !generations !pop !pll_fit ;
