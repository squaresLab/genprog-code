    ...
    let to_print_best_output () =
      ...
      (match !most_fit with
		 | None -> debug "\n\nNo adequate program found.\n" ;
		  debug "v0 %d, " (List.length !v_avg_fit_l);
		  debug "af{i}=[ "; List.iter(fun e -> debug "%g " e)(List.rev !v_avg_fit_l);debug "]; ";
		  debug "bf{i}=[ "; List.iter(fun e -> debug "%g " e)(List.rev !v_bc_fit_l);debug "];\n"
      | Some(best_size, best_fitness, best_file, tau, best_count) -> begin
		  (*v_*)
		  debug "v1 %d, " (List.length !v_avg_fit_l);
		  debug "af{i}= [ "; List.iter(fun e -> debug "%g " e)(List.rev !v_avg_fit_l);debug "]; ";
		  debug "bf{i}= [ "; List.iter(fun e -> debug "%g " e)(List.rev !v_bc_fit_l);debug "];\n";
