...
(*************************************************************************
 *************************************************************************
           Mutational Robustness: Explore the Neutral Landscape
 *************************************************************************
 *************************************************************************)
(* TODO: need to set promut equal to 1 somewhere *)
(* TODO: should use the incoming population *)
let mut_rb (original : 'a Rep.representation) incoming_pop =
  debug "search: mutational robustness random walk begins\n" ;
  let neut_score = (!pos_tests + !neg_tests) in
  debug "search: a fitness of %d is considered neutral\n" neut_score;
  let prev_pop = ref ((original#copy ()) :: []) in
  let prev_pop_index = ref 0 in
  (* return an individual and move on to the next *)
  let variant () =
    let out = List.nth !prev_pop !prev_pop_index in
      incr prev_pop_index ;
      if (!prev_pop_index == List.length !prev_pop) then
        prev_pop_index := 0 ;
      out
  in
  (* choose a stmt uniformly at random *)
  ...
  for step = 1 to !generations do
    (* initialize the population to a bunch of random mutants *)
    let pop = ref [] in
      for i = 1 to !popsize do
        pop := (mutate (variant ()) random) :: !pop
      done ;

    (* test the muntants for neutrality *)
      let neutral =
        List.filter (fun (variant) ->
                       ((test_all_fitness variant) >= (float neut_score)))
          !pop in
      debug "step:%d neutral:%d/%d\n"
        step (List.length neutral) (List.length !pop) ;
      debug "neutral variants:" ;
      List.iter (fun i -> debug " %d" i#test_counter) neutral;
      debug "\n" ;
    ...
    prev_pop := random_order !pop ;
  done ;
  debug "search: mutational robustness neutral walk ends\n" ;
  !prev_pop
