...
liter
(fun stmt_id ->
pos_stmts :=  (my_int_of_string stmt_id) :: !pos_stmts ) (get_lines coverage_outname);
pos_stmts := uniq !pos_stmts;
...
debug "opening %s for path\n" fix_path;
        let fout = open_out fix_path in
          liter
            (fun stmt ->
              let str = Printf.sprintf "%d\n" stmt in
              output_string fout str) (uniq !pos_stmts);
          close_out fout;
debug "done\n";
(*    with _ -> begin
