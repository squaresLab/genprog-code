...
let collect_changes this_rev prev_rev (logmsg) (url) (exclude_regexp) =
  (* project is checked out in benchmark/ *)

  (* get diffs *)
  let saved_dir = Printf.sprintf "%s_saved_files" !benchmark in
  let input = 
    let diff_fin_name = Printf.sprintf "%s/%s-%d.diff" !read_svn_dir !benchmark this_rev in 
    let svn_cmd = "svn diff -x -uw -c "(String.of_int this_rev)^" "^url^" > "^diff_fin_name in 
  ...
  let need_to_look = 
    List.exists 
      (fun (fname,_) -> 
        ...
        let old_fname = Printf.sprintf "%s/%s.c-%d" saved_dir filename prev_rev in
        let new_fname = Printf.sprintf "%s/%s.c-%d" saved_dir filename this_rev in
