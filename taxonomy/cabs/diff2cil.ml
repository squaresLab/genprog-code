...
let convLoc (l : cabsloc) =
  Printf.printf "convloc\n"; Pervasives.flush stdout;
  Printf.printf "filename: %s\n" l.filename; Pervasives.flush stdout;
  Printf.printf "lineno: %d\n" l.lineno; Pervasives.flush stdout;
  Printf.printf "byteno: %d\n" l.byteno; Pervasives.flush stdout;
    Printf.printf "convLoc at %s: line %d, btye %d\n" l.filename l.lineno l.byteno; 
  Pervasives.flush stdout;
  {Cil.line = l.lineno; Cil.file = l.filename; Cil.byte = l.byteno;}
...
let convFile (f : A.file) : Cil.file =
  ...
  List.iter doOneGlobal dl;
	Printf.printf "after do one global\n"; Pervasives.flush stdout;
  let globals = ref (popGlobals ()) in
	Printf.printf "after popGlobals\n"; Pervasives.flush stdout;
...
let convTree tree = 
  ...
  let makeblk (stmts : statement list ) : Cabs.block =
	{ A.blabels = []; A.battrs=[]; A.bstmts = stmts }
  in
  let close_fake fake = 
	let funname = Printf.sprintf "fake_function%d" !fake_num in
	  incr fake_num;
	  FUNDEF(([SpecType(Tvoid)],(funname,JUSTBASE,[],cabslu)),
			 makeblk fake, cabslu,cabslu)
  in
  let asDefs,fake =
	List.fold_left
	  (fun (defs,current_fake_function) ->
		fun tn ->
		  match tn,current_fake_function with 
		  | Globals(dlist),None -> defs@dlist,None
		  | Globals(dlist),Some(fake) -> 
			defs@((close_fake fake) :: dlist),None
		  | Stmts(slist),None -> defs,Some(slist)
		  | Stmts(slist),Some(fake) -> defs,Some(fake@slist)
		  | Exps(elist),None -> defs,Some([COMPUTATION(COMMA(elist),cabslu)])
		  | Exps(elist),Some(fake) -> defs,Some(fake@[COMPUTATION(COMMA(elist), cabslu)])
		  | Syntax(_),_ -> defs,current_fake_function) ([],None) tns
  in
  let defs = 
	match fake with
	  None -> asDefs 
	| Some(fake) -> asDefs @ [close_fake fake]
  in
	Printf.printf "Before convfile\n"; Pervasives.flush stdout;
	convFile (fname,defs)
