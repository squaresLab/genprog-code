...
let add_identifier: (string -> unit) ref = 
  ref (fun _ -> failwith "You called an uninitialized add_identifier")

let add_type: (string -> unit) ref = 
  ref (fun _ -> failwith "You called an uninitialized add_type")

let push_context: (unit -> unit) ref = 
  ref (fun _ -> failwith "You called an uninitialized push_context")
